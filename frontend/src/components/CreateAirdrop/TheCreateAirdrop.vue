      ...
      <main class="main">
        <div class="max-w-[660px] w-full">
          ...
          <form class="form">
            <div class="w-full">
              ...
              >
                <template #left-sidebar>
                  ...
                  <div @click="addHours(168)" class="cursor-pointer left-sidebar-button">
                    1 week
                  </div>
                </template>
              </Datepicker>
            ...
            <div class="w-full">
              <div
                class="message text-[#4D4F55] bg-[#F0F1F5] border-[#C6C9CF] h-[40px] flex items-center pl-[12px]"
              >
        ...
        <template v-if="token && airdropStore.step < 2">
          <div class="mt-[48px] max-w-[660px]">
            ...
            <div ref="dropZoneRef" class="upload-file bg-[#ECF1FE] relative group mt-[20px]">
              <div
                class="w-[96px] h-[96px] bg-transparent group-hover:bg-[#B1C5FA] rounded-full absolute -top-6"
              >
                <span
                  class="upload-sign absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
                >
              ...
              <div
                class="relative text-center mt-5"
          ...
          <div class="table w-full">
            ...
            >
              ...
              >
                <div
                  class="flex items-center px-[12px] border-t border-l border-[#E4E5EA]"
                ...
                <div
                  class="px-[12px] py-[4px] flex items-center justify-center border-t border-[#E4E5EA]"
                  :class="{ 'border-b ': i + 1 === items.length }"
                >
                  <input
                    :disabled="isWaiting()"
                ...
                <div
                  class="px-[12px] py-[4px] flex items-center justify-center border-t border-r border-[#E4E5EA]"
                  :class="{ 'border-b ': i + 1 === items.length }"
                >
                  <input
                    :disabled="isWaiting()"
                ...
                >
                  <div class="h-[40px] flex items-center justify-end px-[12px] space-x-[17px]">
                    <span
                      v-if="hoverItem === i"
                      @click="addItem(i)"
                      class="plusSign cursor-pointer relative left-1"
                    >
                      <PlusIcon />
                    </span>
...
performance.getEntriesByType('navigation').forEach((p, i) => {
  ...
  if (airdropStore.step <= 6) {
    ...
    if (preservedAirdropData !== null) {
      ...
      const retreivedToken = tokensList.find(
        (token) => token.address == preservedAirdropData.tokenRootAddr
      );
  ...
  for (let i = 0; i < tokenAddr.balances.length; i++) {
    ...
    if (token == undefined) {
      ...
      tokenList.value.push({
        label: tokenAddr.balances[i].token,
        decimals: decimal.value0 * 1,
        address: tokenAddr.balances[i].rootAddress,
...
function shouldBeDisabledToken() {
  ...
  return airdropStore.step < 2 && !airdropStore.waiting
    ? recipientStore.updateDropdownVisibility()
    : null;
}
function shouldBeDisabledLock() {
  //console.log('WAITING 2', airdropStore.waiting);
  return airdropStore.step > 1 || airdropStore.waiting ? true : false;
}
function isWaiting() {
  return airdropStore.waiting ? true : false;
}
function disableName() {
  if (airdropStore.step > 1) {
