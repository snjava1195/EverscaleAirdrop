...
{	
	...
	constructor() public {
    	...
    	function deploy(TvmCell _airDropCode, address _refund_destination, address[] _addresses, uint128[] _amounts, uint256 refund_lock_duration_end) external returns(address) {
       	 tvm.accept();
      		uint id = m_deployedNumber;
		uint n = 10;
     		address everAirdrop = new EverAirdrop {
           		code: _airDropCode,
            		value: 1 ever,
            		pubkey: 0,
            		varInit: 
        ...
        }(_refund_destination,_addresses,_amounts,refund_lock_duration_end);
        ++m_deployedNumber;
        return everAirdrop;
    }
